...
import {AppContainer,AppHeading,AppIcon,AppInput,AppListContainer,AppSearchContainer,TabListContainer} from './styled'
...
const tabsList = [
  ...
  const StoreApp =()=>{
    const [tabIdValue,setTabIdValue]= useState(tabsList[0].tabId)

    const [value,setValue]= useState('')

    const onTabItem=(tabId)=>{
      setTabIdValue(tabId)

    }

    const filteredResults = appsList.filter(eachApp=>(
      eachApp.category===tabIdValue
    ))
    ...
    const Results = filteredResults.filter(eachItem=>(
      eachItem.appName.toLowerCase().includes(value.toLowerCase())
    ))

    return(
        <AppContainer>
            <AppHeading>App Store</AppHeading>
            <AppSearchContainer>
            <AppInput placeholder="Search" onChange={onChangeValue} type='search'/>
            <AppIcon src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png" alt=" search icon"/>
            </AppSearchContainer>
            <TabListContainer>
             {tabsList.map(eachTab=>(
                <TabItem eachTab={eachTab} key={eachTab.tabId} onTabItem={onTabItem} isActive={eachTab.tabId===tabIdValue}/>
             ))}
            </TabListContainer>
            <AppListContainer>
                {Results.map(eachApp=>(
